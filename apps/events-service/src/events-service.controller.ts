...
export class EventsServiceController {
  ...
  @Put(':id')
  update(@Param('id', ParseUUIDPipe) id: string, @Body() updateEventDot: UpdateEventDto, @Headers('x-user-id') userId: string, @Headers('x-user-role') userRole: string) {
    return this.eventsServiceService.update(id, updateEventDot, userId, userRole);
  }
