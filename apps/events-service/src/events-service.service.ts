...
export class EventsServiceService implements OnModuleInit {
  constructor(
    @Inject(KAFKA_SERVICE) private readonly kafkaClient: ClientKafka, private readonly dbService: DatabaseService) { }

  private validateEventPermission(organizerId: string, userId: string, userRole: string, action: string) {
    if (organizerId !== userId && userRole !== "ADMIN") {
      throw new ForbiddenException(`You are not authorized to ${action} this event`);
    }
  }
  ...
  async update(id: string, updateEventDto: UpdateEventDto, userId: string, userRole: string) {
    ...
    this.validateEventPermission(event.organizerId, userId, userRole, 'update')
  ...
  async publish(id: string, userId: string, userRole: string) {
    ...
    this.validateEventPermission(event.organizerId, userId, userRole, 'publish')
  ...
  async cancel(id: string, userId: string, userRole: string) {
    ...
    this.validateEventPermission(event.organizerId, userId, userRole, 'cancel')
