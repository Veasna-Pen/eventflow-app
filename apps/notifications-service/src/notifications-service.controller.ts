...
export class NotificationsServiceController {
  ...
  async handleTicketPurchased(@Payload() data: { ticketId: string; ticketCode: string; userId: string; email?: string; eventTitle?: string; quantity: number; totalPrice: number }) {
  ...
  async handleTicketCancelled(@Payload() data: { ticketId: string; userId: string; email?: string }) {
    this.logger.log(`Received ticket cancellation event: ${JSON.stringify(data)}`);
