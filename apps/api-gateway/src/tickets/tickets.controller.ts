...
export class TicketsController {
    ...
    @Post('purchase')
    purchase(@Body() purchaseDto: PurchaseTicketDto, @Request() req: { user: { userId: string } }) {
    ...
    @Get(':id')
    findOne(@Param('id', ParseUUIDPipe) id: string, @Request() req: { user: { userId: string } }) {
        return this.ticketsService.findOne(id, req.user.userId);
    }

    @Post(':id/cancel')
    cancel(@Param('id', ParseUUIDPipe) id: string, @Request() req: { user: { userId: string } }) {
        return this.ticketsService.cancel(id, req.user.userId);
    }

    @Post('check-in')
    checkIn(@Body() checkInDto: CheckInTicketDto, @Request() req: { user: { userId: string } }) {
        return this.ticketsService.checkIn(checkInDto, req.user.userId);
    }

    @Get('event/:eventId')
    findEventTickets(@Param('eventId', ParseUUIDPipe) eventId: string, @Request() req: { user: { userId: string } }) {
