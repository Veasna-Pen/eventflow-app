...
export class TicketsServiceController {
  ...
  @Post('purchase')
  purchase(@Body() purchaseDto: PurchaseTicketDto, @Headers('x-user-id') userId: string) {
  ...
  @Get(':id')
  findOne(@Param('id', ParseUUIDPipe) id: string, @Headers('x-user-id') userId: string) {
  ...
  @Post('check-in')
  checkIn(@Body() checkInDto: CheckInTicketDto, @Headers('x-user-id') organizerId: string) {
    return this.ticketsServiceService.checkIn(checkInDto.ticketCode, organizerId);
  }

  @Get('event/:eventId')
  findEventTickets(@Param('eventId', ParseUUIDPipe) eventId: string, @Headers('x-user-id') userId: string) {
    return this.ticketsServiceService.findEventTicket(eventId, userId);
  }
}
